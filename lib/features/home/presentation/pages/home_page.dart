...
class _HomePageState extends State<HomePage> {
        ...
        BlocListener<ReportBloc, ReportState>(
          ...
          listener: (context, state) {
            if (state.selectedId != null) {
              // Dışarıdan bir seçim geldi (örn. bildirim), detay görünümünü aç
              setState(() {
                ...
                Expanded(
                  child: BlocBuilder<ReportBloc, ReportState>(
                    builder: (context, state) {
                      ...
                      // Otomatik seçim yapma - sadece manuel tıklamada seçilsin
                      _selected = selected;
  ...
  Widget _mapSection(List<ReportEntity> reports, ReportEntity? selected) {
    final mapHeight =
        ...
        0.42; // roughly 65% of remaining vertical chunk with list
    final center =
        selected?.location ??
        (reports.isNotEmpty
            ? reports.first.location
            : LatLng(39.9069, 41.2779));
  ...
  Widget _pinCard(ReportEntity report) {
                  ...
                  Expanded(
                    child: Column(
                      ...
                      children: [
                        ...
                        Text(
                          '${_typeLabel(report.type)} · ${df.format(report.createdAt)}',
                          style: const TextStyle(
  ...
  void _openDetail(ReportEntity report) {
    Navigator.push(
      context,
      MaterialPageRoute(
        builder: (_) => BlocProvider.value(
